...
parser.add_argument(
    ...
    type=str,
    default="None",
    help="Adjust learning rate, options=[None, StepLR]"
)
...
parser.add_argument(
    ...
    required=True,
    type=str,
    help="Evaluation metrics, options=[Accuracy, Precision, Recall, F1-score, P@min, r-Precision, AUC, TPR, FPR, TP, FP, TN, FN]"
)
parser.add_argument(
    "--save_metric",
    type=str,
...
print("loading train file:", os.path.join(in_path, f"{args.train_file}.npz"))
train_X, train_y = data_processor.load_data(
    os.path.join(in_path, f"{args.train_file}.npz"),
    args.feature,
    args.seq_len,
    args.num_tabs
)
valid_X, valid_y = data_processor.load_data(
    os.path.join(in_path, f"{args.valid_file}.npz"),
    args.feature,
    args.seq_len,
    args.num_tabs
)
...
if args.load_file is None:
    ...
    checkpoint = torch.load(args.load_file, map_location="cpu")
