# We made minor modifications to the code from
# https://github.com/SPIN-UMass/Realistic-Website-Fingerprinting-By-Augmenting-Network-Traces
import sys
import os
import torch
import random
...
random_X = train_X[np.random.choice(range(len(train_X)), size=sample_size, replace=False)]
for x in random_X:
...
OUTGOING_BURST_SIZE_CDF = np.zeros_like(bins, dtype=np.float64)
...
netclr = netclr_pretrain.NetCLR(
    ...
    optimizer=optimizer,
    scheduler=scheduler,
    fp16_precision=(device.type == "cuda"),
    temperature=0.5,
    n_views=2,
